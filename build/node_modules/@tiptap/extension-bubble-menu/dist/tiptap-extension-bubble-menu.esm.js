...
var tippy_esm = require('../../../tippy.js/dist/tippy.esm.js');
    ...
    constructor({ editor, element, view, tippyOptions = {}, shouldShow, }) {
        ...
        this.shouldShow = ({ view, state, from, to, }) => {
            ...
            const isEmptyTextBlock = !doc.textBetween(from, to).length
                && tiptapCore_esm.isTextSelection(state.selection);
            if (!view.hasFocus()
                || empty
                || isEmptyTextBlock) {
                return false;
    ...
    update(view, oldState) {
        ...
        (_b = this.tippy) === null || _b === void 0 ? void 0 : _b.setProps({
            getReferenceClientRect: ((_c = this.tippyOptions) === null || _c === void 0 ? void 0 : _c.getReferenceClientRect) || (() => {
                if (tiptapCore_esm.isNodeSelection(state.selection)) {
                    const node = view.nodeDOM(from);
                    if (node) {
                        return node.getBoundingClientRect();
                    }
                }
                return tiptapCore_esm.posToDOMRect(view, from, to);
            }),
        });
        this.show();
    }
...
const BubbleMenuPlugin = (options) => {
    return new index.Plugin({
        key: typeof options.pluginKey === 'string'
            ? new index.PluginKey(options.pluginKey)
            : options.pluginKey,
        view: view => new BubbleMenuView({ view, ...options }),
    });
};

tiptapCore_esm.Extension.create({
    name: 'bubbleMenu',
    addOptions() {
        return {
            element: null,
