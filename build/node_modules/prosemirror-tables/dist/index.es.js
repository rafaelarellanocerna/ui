...
var index$2 = require('../../prosemirror-keymap/dist/index.js');
var index$3 = require('../../prosemirror-view/dist/index.js');
var index$4 = require('../../prosemirror-transform/dist/index.js');
...
var key = new index.PluginKey("selectingCells");
...
var CellSelection = /*@__PURE__*/(function (Selection) {
  ...
  CellSelection.prototype.content = function content () {
    ...
    for (var row = rect.top; row < rect.bottom; row++) {
      ...
      rows.push(table.child(row).copy(index$1.Fragment.from(rowContent)));
    }

    var fragment = this.isColSelection() && this.isRowSelection() ? table : rows;
    return new index$1.Slice(index$1.Fragment.from(fragment), 1, 1)
  };

  CellSelection.prototype.replace = function replace (tr, content) {
    if ( content === void 0 ) content = index$1.Slice.empty;
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      tr.replace(mapping.map($from.pos), mapping.map($to.pos), i ? index$1.Slice.empty : content);
    }
  ...
  CellSelection.prototype.replaceWith = function replaceWith (tr, node) {
    this.replace(tr, new index$1.Slice(index$1.Fragment.from(node), 0, 0));
  };
...
CellBookmark.prototype.resolve = function resolve (doc) {
  ...
  if ($anchorCell.parent.type.spec.tableRole == "row" &&
    ...
    { return index.Selection.near($headCell, 1) }
};
...
function drawCellSelection(state) {
  ...
  state.selection.forEachCell(function (node, pos) {
    cells.push(index$3.Decoration.node(pos, pos + node.nodeSize, {class: "selectedCell"}));
  });
  return index$3.DecorationSet.create(state.doc, cells)
}
...
function normalizeSelection(state, tr, allowTableNodeSelection) {
  ...
  } else if (sel instanceof index.TextSelection && isCellBoundarySelection(sel)) {
    normalize = index.TextSelection.create(doc, sel.from);
  } else if (sel instanceof index.TextSelection && isTextSelectionAcrossCells(sel)) {
    normalize = index.TextSelection.create(doc, sel.$from.start(), sel.$from.end());
  }
  if (normalize)
...
function pastedCells(slice) {
  ...
  if (role == "row") {
    for (var i = 0; i < content.childCount; i++) {
      ...
      if (left || right) { cells = fitSlice(tableNodeTypes(schema).row, new index$1.Slice(cells, left, right)).content; }
      rows.push(cells);
    }
  } else if (role == "cell" || role == "header_cell") {
    rows.push(openStart || openEnd ? fitSlice(tableNodeTypes(schema).row, new index$1.Slice(content, openStart, openEnd)).content : content);
  } else {
    return null
  }
...
function fitSlice(nodeType, slice) {
  ...
  var tr = new index$4.Transform(node).replace(0, node.content.size, slice);
  return tr.doc
}
...
function clipCells(ref, newWidth, newHeight) {
  ...
  if (width != newWidth) {
    ...
    for (var row = 0; row < rows.length; row++) {
      ...
      newRows.push(index$1.Fragment.from(cells));
    }
    rows = newRows;
    width = newWidth;
  }
  ...
  if (height != newHeight) {
    ...
    for (var row$1 = 0, i$1 = 0; row$1 < newHeight; row$1++, i$1++) {
      ...
      newRows$1.push(index$1.Fragment.from(cells$1));
    }
    rows = newRows$1;
    height = newHeight;
  }
...
function growTable(tr, map, table, start, width, height, mapFrom) {
  ...
  if (height > map.height) {
    ...
    var emptyRow = types.row.create(null, index$1.Fragment.from(cells$1)), rows = [];
...
function insertCells(state, dispatch, tableStart, rect, cells) {
  ...
  for (var row = top; row < bottom; row++) {
    ...
    tr.replace(tr.mapping.slice(mapFrom).map(from + tableStart), tr.mapping.slice(mapFrom).map(to + tableStart),
               new index$1.Slice(cells.rows[row - top], 0, 0));
  }
  recomp();
...
var handleKeyDown = index$2.keydownHandler({
...
function arrow(axis, dir) {
  return function (state, dispatch, view) {
    ...
    if (sel instanceof CellSelection) {
      return maybeSetSelection(state, dispatch, index.Selection.near(sel.$headCell, dir))
    }
    if (axis != "horiz" && !sel.empty) { return false }
    var end = atEndOfCell(view, axis, dir);
    if (end == null) { return false }
    if (axis == "horiz") {
      return maybeSetSelection(state, dispatch, index.Selection.near(state.doc.resolve(sel.head + dir), dir))
    } else {
    ...
    sel.forEachCell(function (cell, pos) {
      if (!cell.content.eq(baseContent))
        { tr.replace(tr.mapping.map(pos + 1), tr.mapping.map(pos + cell.nodeSize - 1),
                   new index$1.Slice(baseContent, 0, 0)); }
    });
...
function handlePaste(view, _, slice) {
  ...
  if (sel instanceof CellSelection) {
    if (!cells) { cells = {width: 1, height: 1, rows: [index$1.Fragment.from(fitSlice(tableNodeTypes(view.state.schema).cell, slice))]}; }
...
function atEndOfCell(view, axis, dir) {
  ...
  for (var d = $head.depth - 1; d >= 0; d--) {
    var parent = $head.node(d), index$1 = dir < 0 ? $head.index(d) : $head.indexAfter(d);
    if (index$1 != (dir < 0 ? 0 : parent.childCount)) { return null }
...
var fixTablesKey = new index.PluginKey("fix-tables");
...
function mergeCells(state, dispatch) {
  ...
  if (dispatch) {
    var tr = state.tr, seen = {}, content = index$1.Fragment.empty, mergedPos, mergedCell;
    for (var row = rect.top; row < rect.bottom; row++) {
      for (var col = rect.left; col < rect.right; col++) {
...
var key$1 = new index.PluginKey("tableColumnResizing");
...
function handleDecorations(state, cell) {
  ...
  for (var row = 0; row < map.height; row++) {
    ...
    if ((col == map.width || map.map[index] != map.map[index + 1]) &&
      ...
      decorations.push(index$3.Decoration.widget(pos, dom));
    }
  }
  return index$3.DecorationSet.create(state.doc, decorations)
}
